    ...
    for (const key of keys) {
      if (typeof assessment[key as keyof AssessmentInt] === "object") {
        switch (key) {
              ...
              trs.push(
                <tr>
                  ...
                  {Object.keys(building).map((key: string) => {
                    return (
                      <td>
                        {key}:{building[key as keyof Building]}
                      </td>
                    );
                  })}
                </tr>,
          ...
          case "ownerName": {
            if (
              assessment.ownerName.length == 1 ||
              !assessment.ownerName.length
            ) {
              trs.push(
                <tr>
                  <td>owner names</td>
                ...
                <tr>
                  ...
                  {keys.map((key, index) => {
                    if (index == 0) {
                      return (
                        <td colSpan={4}>
                          {key}: {garage[key as keyof Garage]}
                        </td>
                      );
                    } else {
                      return (
        ...
        if (assessment[key as keyof AssessmentInt]?.toString().length) {
          trs.push(
            <tr>
              <td>{key.toString()}</td>
              <td colSpan={100}>
                {assessment[key as keyof AssessmentInt]?.toString()}
              </td>
            </tr>,
          );
        }
        ...
        return (
          <>
            ...
            <Table striped bordered hover responsive="sm" variant={theme}>
              <tbody>{returnTable(assessment)}</tbody>
            </Table>
          </>
        );
      })}
