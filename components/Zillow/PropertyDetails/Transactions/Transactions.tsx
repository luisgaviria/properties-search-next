    ...
    for (const key of keys) {
      if (typeof transaction[key as keyof TransactionInt] === "object") {
        switch (key) {
              ...
              trs.push(
                <tr>
                  ...
                  {keys.map((key) => {
                    return (
                      <td>
                        {key}: {parcel[key as keyof Parcel]}
                      </td>
                    );
                  })}
                </tr>,
          ...
          case "lenderName": {
            if (
              transaction.lenderName?.length == 1 ||
              !transaction.lenderName?.length
            ) {
              trs.push(
                <tr>
                  <td>lender names</td>
        ...
        if (transaction[key as keyof TransactionInt]?.toString().length) {
          trs.push(
            <tr>
              <td>{key.toString()}</td>
              <td colSpan={100}>
                {transaction[key as keyof TransactionInt]?.toString()}
              </td>
            </tr>,
          );
        }
        ...
        return (
          <>
            ...
            <Table striped bordered hover responsive="sm" variant={theme}>
              <tbody>{returnTable(transaction)}</tbody>
            </Table>
          </>
        );
      })}
